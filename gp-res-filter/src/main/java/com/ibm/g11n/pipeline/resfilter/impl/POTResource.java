 ...
 *
 */
public class POTResource extends ResourceFilter {
    ...
    /**
     * Note:
     * {@link POTResource#merge(InputStream, OutputStream, LanguageBundle, FilterOptions)}
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (line.startsWith(UNTRANSLATED_STRING_PREFIX)) {
                ...
                if (!line.startsWith(TRANSLATED_STRING_PREFIX) || key == null || key.isEmpty()
                        || !kvMap.containsKey(key) || kvMap.get(key) == null) {
                    ...
                    writer.write(formatMessage(TRANSLATED_STRING_PREFIX, kvMap.get(key), brkItr));
                    writer.newLine();
     ...
     * "multi-line message."<br>
     */
    static String formatMessage(String prefix, String message, BreakIterator brkItr) {
        int maxLineLen = 80;
