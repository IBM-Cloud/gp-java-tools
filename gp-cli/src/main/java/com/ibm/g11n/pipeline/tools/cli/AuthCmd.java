...
/**
 * A base class of Globalization Pipeline commands that require authentication.
 * 
 * @author Yoshito Umaoka
 */
public abstract class AuthCmd extends BaseCmd {
    @Parameter(
            names = {"-u", "--user"},
            description = "User ID",
            required = true)
    ...
    protected ServiceClient getClient() {
        ServiceAccount account = ServiceAccount.getInstance(
                gpUrl, "zzz" /* dummy instance ID for now */,
                userId, password);
